...
void SdManager::begin() {
    // SPI.begin() is called exactly once here and never again.
    // Re-calling SPI.begin() on an already-inited bus can corrupt
    // in-flight transactions on the shared bus.
    if (!_spiBegun) {
